...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $con->prepare("INSERT INTO application_table 
            (institutionInput,administrative_details,research_related_info,participant_data,benefit_risk,informed_consent,compensation_data,storage_confidentality,academic_research,research_type,consent_type,waiver_reason,documents_files,user_id) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

        $stmt->bind_param("ssssssssssssss",$institutionInput,$administrative_details,$research_related_info,$participant_data,$benefit_risk,$informed_consent,$compensation_data,$storage_confidentality,$academic_research,$research_type,$consent_type,$waiver_reason,$documents_files,$user_id);
