...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    'consentFile' => uploadFile('consent_obtain_other_upload', $uploadDir),
    'mouFile' => uploadFile('MoU_upload', $uploadDir),
    'Proposal_pdf_copy' => uploadFile('Proposal_pdf_copy', $uploadDir),
    'ctri_request_upload' => uploadFile('ctri_request_upload', $uploadDir),
    ]);
    ...
    try {
        ...
        $stmt = $con->prepare("INSERT INTO application_table 
            (institutionInput,administrative_details,research_related_info,participant_data,benefit_risk,informed_consent,compensation_data,storage_confidentality,academic_research,research_type,consent_type,waiver_reason,documents_files,user_id,in_status) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

        $stmt->bind_param("sssssssssssssss",$institutionInput,$administrative_details,$research_related_info,$participant_data,$benefit_risk,$informed_consent,$compensation_data,$storage_confidentality,$academic_research,$research_type,$consent_type,$waiver_reason,$documents_files,$user_id,$in_status);

        if (!$stmt->execute()) {
            throw new Exception("Error inserting main form: " . $stmt->error);
        }
